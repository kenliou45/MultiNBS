...
def consensus_hclust_hard(Hlist, k=3, hclust_linkage_method='average',
    ...
    for H in Hlist:
        ...
        cluster_count.loc[H.index, H.index]+=1  # used to be .ix
        # Get cluster assignment for each patient
        cluster_assign = {column:[] for i, column in enumerate(H.columns, start=1)} # changed from just using another for loop
        for pat in H.index:
            cluster_assign[H.columns[np.argmax(H.loc[pat])]].append(pat)  # used to be .ix changed this from without the H.columns
        # Update co-clustering matrix with each cluster assignment
        for cluster in cluster_assign:
            cluster_pats = cluster_assign[cluster]
            co_clust_table.loc[cluster_pats, cluster_pats]+=1 # used to be .ix
