...
def query_batch(query_string, tax_id='9606', scopes="symbol, entrezgene, alias, uniprot", fields="symbol, entrezgene"):
	...
	if query_n <=1000:
		...
		chunks = int(chunks)  # Convert 'chunks' to an integer if it contains a valid numerical value
...
def construct_query_map_table(query_result, query_genes, display_unmatched_queries=False):
	...
	for gene in matched_genes:
		if type(match_table.loc[gene])==pd.DataFrame:  # .ix
	...
	match_table_single = match_table.loc[single_match_genes]  # .ix
	...
	if len(duplicate_matched_genes) > 0:
		...
		for gene in duplicate_matched_genes:
			matched_duplicates = match_table.loc[gene] #.ix
...
def convert_custom_namelist(names, field, match_table):
	...
	if field=='symbol':
		...
		conversion = match_table.loc[names][~(match_table.loc[names]['Symbol'].isnull())] # 2 .ix
		if conversion.shape[0]==0:
			return None
		else:
			# Return conversion with max score or None if no conversion
			max_score = conversion['Score'].max()
			converted_namelist = conversion[conversion['Score']==max_score].iloc[0]['Symbol'] # .ix
	elif field=='entrez':
		# Return match table values that have matched symbol
		conversion = match_table.loc[names][~(match_table.loc[names, 'EntrezID'].isnull())] # .ix
		if conversion.shape[0]==0:
			return None
		else:
			# Return conversion with max score or None if no conversion
			max_score = conversion['Score'].max()
			converted_namelist = conversion[conversion['Score'] == max_score].iloc[0]['EntrezID'] # .ix
