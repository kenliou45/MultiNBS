...
def network_inf_KNN_glap(network, gamma=0.01, kn=11, verbose=True, **save_args):
    ...
    for gene in L_inv.index:
        gene_knn = L_inv.loc[gene].sort_values(ascending=False)[:kn].index  # used to be .ix
        for neighbor in gene_knn:
            if L_inv.loc[gene, neighbor] > 0:  # used to be L_inv.ix[gene][neighbor]
...
def subsample_sm_mat(sm_mat, propNet=None, pats_subsample_p=0.8, gene_subsample_p=0.8, min_muts=10):          
    ...
    # Sub sample patients
    pats_subsample = random.sample(list(sm_mat.index), int(Nsample))  # added list in front
    # Sub sample genes
    gene_subsample = random.sample(list(sm_mat.columns), int(Dsample)) # added list in front 
    # Sub sampled data mat
    gind_sample = sm_mat.loc[pats_subsample, gene_subsample]  # also used to be .ix here
    ...
    if propNet is not None:
        ...
        gind_sample_filt = gind_sample.T.loc[gind_sample.T.index.isin(list(propNet.nodes))].reindex(list(propNet.nodes)).fillna(0).T # used to be .ix
    else:
...
def mixed_netNMF(data, KNN_glap, k=3, l=200, maxiter=250, 
    ...
    # Initialize W
    W_init = np.linalg.lstsq(H.T, data.T, rcond=-1)[0].T  # suppresses warning/uses value close to machine precision 
