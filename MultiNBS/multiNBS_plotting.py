...
def cluster_color_assign(cluster_assignments, name=None):
    ...
    for pat in cluster_assignments.index:
        pat_colors[pat] = cluster_cmap[cluster_assignments.loc[pat]] # .ix
...
def cluster_KMplot(cluster_assign, clin_data_fn, delimiter='\t', lr_test=True, tmax=-1, verbose=True, **save_args):
    ...
    for clust in clusters:
        ...
        clust_surv_data = surv.loc[clust_pats].dropna() # .ix
    ...
    if lr_test:
        ...
        stat = multiv_lr_test(np.array(cluster_survivals.overall_survival), 
                           np.array(cluster_survivals[cluster_assign.name]), t_0=tmax,
                           event_observed=np.array(cluster_survivals.vital_status)).test_statistic
        if verbose:
