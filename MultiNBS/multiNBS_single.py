...
def NBS_single(sm_mat, regNet_glap, propNet=None, propNet_kernel=None, 
    ...
    if propNet is not None:
        ...
        data_arr = np.array(prop_data_qnorm.T.loc[propNet_nodes]) # .ix
        regNet_glap_arr = np.array(regNet_glap.loc[propNet_nodes][propNet_nodes])  # .ix
    else:
        propNet_nodes = list(regNet_glap.index)
        data_arr = np.array(prop_data_qnorm.T.loc[prop_data_qnorm.T.index.isin(propNet_nodes)].fillna(0).T) # .ix binary_matrix.T.index.isin(prop_nodelist) 
