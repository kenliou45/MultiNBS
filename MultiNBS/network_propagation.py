...
def network_propagation(network, binary_matrix, alpha=0.7, symmetric_norm=False, verbose=True, **save_args):                        
    ...
    subgraphs = [network.subgraph(component) for component in nx.connected_components(network)] # subgraphs = list(nx.connected_component_subgraphs(network))
    # Initialize propagation results by propagating first subgraph
    subgraph = subgraphs[0]
    subgraph_nodes = list(subgraph.nodes)
    prop_data_node_order = list(subgraph_nodes)
    binary_matrix_filt = np.array(binary_matrix.T.loc[subgraph_nodes].fillna(0).T) # .ix 
...
def network_kernel_propagation(network, network_kernel, binary_matrix, verbose=False, **save_args):
    ...
    prop_data = np.dot(binary_matrix.T.loc[binary_matrix.T.index.isin(prop_nodelist)].fillna(0).T,     # prop_nodelist
                       network_kernel.loc[prop_nodelist][prop_nodelist]) # 2.ix  is this matrix multiplication correct?
    # Get propagated results for remaining subgraphs
    for nodelist in subgraph_nodelists[1:]:
        ...
        subgraph_prop_data = np.dot(binary_matrix.T.loc[subgraph_nodes].fillna(0).T, 
                                    network_kernel.loc[subgraph_nodes][subgraph_nodes]) # 2 .ix
